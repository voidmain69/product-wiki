...
const TO_SI: Record<string, { unit: string; factor: number }> = {
  ...
  in: { unit: "inch", factor: 1 }, inch: { unit: "inch", factor: 1 },
...
const SCALAR = /^\s*(?:[<>≤≥~]|до|макс\.?|мін\.?|max|min|прибл\.?|близько|≈)?\s*(-?\d[\d\s ]*(?:[.,]\d+)?)\s*([%°″"'a-zа-яіїєґ]{1,6}\.?)?\s*$/i;
...
export function guessUnit(label: string): string | null {
  ...
  if (has(/ширина|висота|глибина|довжина|діаметр|товщина|width|height|depth|length/) && !has(/екран|дисплей|піксел/)) return "mm";
